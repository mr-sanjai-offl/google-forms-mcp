        ...
        for item in item_list:
            ...
            if "validation" in item:
                ...
                if not isinstance(val, dict) or "type" not in val:
                    errors.append(
                        f"Invalid validation format for question '{title}'. Must have a 'type'."
                    )
...
def estimate_completion(question_count: int, has_branching: bool, has_validation: bool) -> str:
    ...
    if has_branching:
        api_calls += (
            question_count // 2
        )  # Rough estimate: half the questions have branching updates
    ...
    return json.dumps(
        {"estimated_api_calls": api_calls, "estimated_time_seconds": round(estimated_seconds, 1)},
        indent=2,
    )
...
def register_planning_tools(mcp: Any) -> None:
    ...
    """
    mcp.tool()(validate_execution_plan)
    mcp.tool()(estimate_completion)
